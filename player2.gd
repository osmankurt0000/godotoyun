...
var jump_force = -500  # Zıplama kuvveti (daha güçlü zıplama için artırıldı)
...
func _physics_process(delta: float) -> void:
	...
	if is_crouching:
		...
		if direction == Vector2.ZERO:
			animated_sprite_2d.play("player2")  # Karakter hareket etmiyorsa 'player2' animasyonunu oynat
		else:
			# X yönüne göre karakteri döndürme
			if direction.x < 0:
			...
			if not is_on_floor():
				...
				animated_sprite_2d.play("run")  # Zemin üzerindeyken koşma animasyonu
